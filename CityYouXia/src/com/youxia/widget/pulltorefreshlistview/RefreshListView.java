...
import com.youxia.R;
import com.youxia.widget.ScanningImageView;
...
public class RefreshListView extends ListView implements OnScrollListener {
	...
	private ScanningImageView mProgressBar; // 头布局的进度条
	...
	private View noMoreItemView; // 没有更多布局
	private int noMoreItemViewHeight; //没有更多高度
	private boolean isLoadingMore = false; // 是否正在加载更多中

	private boolean isDownRefresh = true;//是否允许下拉刷新
	 ...
	 * 不允许下拉刷新
	 * */
	public void disableDownRefresh(){
		isDownRefresh = false;
	}
	 ...
	 * 初始化没有更多消息布局
	 */
	private void initNoMoreItemView() {
		noMoreItemView = View.inflate(getContext(), R.layout.refreshlistview_footer_nomore, null);
		noMoreItemView.measure(0, 0);
		noMoreItemViewHeight = noMoreItemView.getMeasuredHeight();
		noMoreItemView.setPadding(0, -noMoreItemViewHeight, 0, 0);
		this.addFooterView(noMoreItemView);
	}
	...
	private void initHeaderView() {
		...
		mProgressBar = (ScanningImageView) headerView.findViewById(R.id.pb_listview_header_progress);
		...
		case MotionEvent.ACTION_MOVE:
			if(isDownRefresh) {//是否允许下拉刷新
				...
				if (firstVisibleItemPosition == 0 && -headerViewHeight < paddingTop) {
					...
					//return true;	//这句会影响下拉刷新，ACTION_UP后触发点击时间,去掉这句就行
				}
			}
			break;
	...
	public void onScrollStateChanged(AbsListView view, int scrollState) {
		...
		if (scrollState == SCROLL_STATE_IDLE || scrollState == SCROLL_STATE_FLING) {
			...
			// 判断当前是否已经到了底部
			if (isScrollToBottom && !isLoadingMore && currentState != RELEASE_REFRESH && currentState != REFRESHING) {
				isLoadingMore = true;
				// 当前到底部
				Log.i(TAG, "加载更多数据");
	...
	public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
		...
		int a = getLastVisiblePosition();
		if (a == (totalItemCount - 1)) {
	...
	public void showNoMoreView() {
		noMoreItemView.setPadding(0, 0, 0, 0);
		isLoadingMore = false;
	}
